// ==UserScript==
// @name         Starfleet Commander Galaxy Scan Comparison (v4.6)
...
(function() {
    ...
    console.log("SFCScript: Galaxy Scan Comparison loaded (v4.6).");
                ...
                if (levelCell && fleetNameElement && playerElement && planetLink) {
                    ...
                    if (isDifficultyValid && isNameValid) {
                        ...
                        if (planet && !isNaN(planet)) {
                            ...
                            console.log(`SFCScript: Found valid NPC with key: ${npcKey}`);
                            npcData[npcKey] = {
                                name: fleetName,
    ...
    function displayNewTargets(targets, coords, previousTimestamp) {
        ...
        const targetListHtml = targets.map(target =>
            `<li>${target.name} (Difficulty ${target.difficulty}) - ${target.coords}</li>`
        ).join('');
    ...
    function addFloatingPanel() {
        ...
        const checkButton = createButton('Check last scan', checkLastScan, '#5bc0de'); // New button
