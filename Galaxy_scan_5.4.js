// ==UserScript==
// @name         Starfleet Commander Galaxy Scan Comparison (v5.4)
...
(function() {
    ...
    console.log("SFCScript: Galaxy Scan Comparison loaded (v5.4).");
    ...
    function scrapeNpcData() {
        try {
            ...
            const npcData = {};
            // Select all fleet rows that contain a fleet name and a player element (which contains the difficulty).
            // This is the most reliable way to find all types of NPCs.
            const fleetRows = document.querySelectorAll('.fleet_row:has(.fleet_name):has(.player)');
            console.log(`SFCScript: Found ${fleetRows.length} potential NPC rows.`);
            ...
            fleetRows.forEach(row => {
                ...
                if (levelCell && fleetNameElement && planetLink) {
                    const difficultySpan = playerElement ? playerElement.querySelector('span') : null;
                    const difficultyClass = difficultySpan ? difficultySpan.className : 'easy';
                    let finalDifficulty;
                    switch (difficultyClass) {
                        ...
                        case 'hard':
                            ...
                            break;
                        default:
                            finalDifficulty = -25; // Default to -25 if no class found
                    }
                    ...
                    console.log(`SFCScript: An NPC fleet row was found but one or more required elements were missing.`);
                }
            });
    ...
    function displayNewTargets(newTargets, changedTargets, coords, previousTimestamp) {
        ...
        const changedTargetsListHtml = changedTargets.map(target =>
            `<li>Changed: ${target.target.name} (Old Difficulty ${target.oldDifficulty} -> New Difficulty ${target.newDifficulty}) - ${target.target.coords}</li>`
        ).join('');
