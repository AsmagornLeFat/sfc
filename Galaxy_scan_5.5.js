// ==UserScript==
// @name         Starfleet Commander Galaxy Scan Comparison (v5.5)
...
(function() {
    ...
    console.log("SFCScript: Galaxy Scan Comparison loaded (v5.5).");
    ...
    function scrapeNpcData() {
        try {
            ...
            const npcData = {};
            // Select all fleet rows AND planet rows that are NPCs
            const npcRows = document.querySelectorAll(
                'tr.fleet_row:has(.fleet_name):has(.player), ' + // Old-style NPC fleets
                'tr.planet:has(.name):has(.player)' // New-style NPC planets
            );
            console.log(`SFCScript: Found ${npcRows.length} potential NPC rows.`);

            let npcsFound = 0;
            npcRows.forEach(row => {
                const fleetNameElement = row.querySelector('.fleet_name') || row.querySelector('.name > span');
                const playerElement = row.querySelector(".player");
                const planetLink = row.querySelector('a[href*="/galaxy/"]');

                if (fleetNameElement && playerElement && planetLink) {
